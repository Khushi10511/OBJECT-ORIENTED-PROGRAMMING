...
// a. Find total marks and percentage of all n students.
void findTotalMarksPercentageStudents(struct Student
										  students[],
									  int numberStudents)
{
	int i, m;
...
// b. Display details of a student with a given roll number.
void displayDetailsStudent(struct Student students[], int numberStudents, int roll)
{
	int i, m;
	for (i = 0; i < numberStudents; i++)
	{
		if (roll == students[i].roll)
		{
			...
			printf("The student's rollnumber: %d\n", students[i].roll);
...
void main()
{
	...
	scanf("%d", &roll);
	displayDetailsStudent(students, numberStudents, roll);
